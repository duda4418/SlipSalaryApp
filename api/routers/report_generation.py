...
def create_aggregated_employee_data(managerId: UUID, year: int, month: int, includeBonuses: bool = True, db: Session = Depends(session.get_db)):
...
def send_aggregated_employee_data(managerId: UUID, year: int, month: int, db: Session = Depends(session.get_db)):
    return send_manager_csv(db, managerId, year, month)

@report_generation_router.post("/createPdfForEmployees")
def create_pdf_for_employees(managerId: UUID, year: int, month: int, overwriteExisting: bool = False, db: Session = Depends(session.get_db)):
...
def send_pdf_to_employees(managerId: UUID, year: int, month: int, regenerateMissing: bool = False, db: Session = Depends(session.get_db)):
