...
def create_month_endpoint(payload: MonthInfoCreate, db: Session = Depends(session.get_db)):
    month = svc_create_month(db, payload.model_dump())
    return month

@months_router.put("/{month_id}", response_model=MonthInfoResponse)
def update_month_endpoint(month_id: str, payload: MonthInfoUpdate, db: Session = Depends(session.get_db)):
    data = {k: v for k, v in payload.model_dump().items() if v is not None}
    month = svc_update_month(db, month_id, data)
    return month
